...
const DEFAULT_SIDEBAR_AUTO_SETTLE_AFTER_DAYS: SidebarAutoSettleAfterDays = 7;
...
const DEFAULT_APPEARANCE_CONTRAST: AppearanceContrast = 120;
...
const DEFAULT_TERMINAL_FONT_SIZE: TerminalFontSize = 14;
...
const DEFAULT_VOICE_LANGUAGE_PREFERENCE: VoiceLanguagePreference = "auto";
...
export const ClientSettingsSchema = Schema.Struct({
  ...
  // can settle the composer into its single-line layout. Losing focus never
  // does, and the remaining trigger is opt-in in Scient.
...
export const AntigravitySettings = makeProviderSettingsSchema(
  {
    ...
    binaryPath: TrimmedString.pipe(
      ...
      Schema.annotateKey({
        title: "Binary path",
        description:
          "Optional path to Antigravity. Leave empty for Scient's official ACP runtime; an existing agy path keeps the legacy compatibility runtime.",
...
export const ServerSettings = Schema.Struct({
   ...
   * withholds the `preview` capability from provider sessions. The user's own
   ...
   *
   * Scient enables preview-browser access by default. A user can turn it off
   * explicitly from Integrations. Project Sources and document-building
   * capabilities are baseline Scient features and are not controlled here.
   */
