...
describe("third-party license generation", () => {
  ...
  it("excludes Scient workspace packages from third-party notices", async () => {
    const fixture = await createFixture();
    const firstPartyRoot = NodePath.join(
      fixture.root,
      "node_modules",
      "@scientfactory",
      "internal-fixture",
    ...
    await writeJson(fixture.appManifest, {
      name: "fixture-app",
      dependencies: {
        "@scientfactory/internal-fixture": "workspace:*",
        "demo-dependency": "1.2.3",
      },
    });
    await writeJson(NodePath.join(firstPartyRoot, "package.json"), {
      name: "@scientfactory/internal-fixture",
      version: "0.0.0",
      private: true,
    });
    ...
    expect(manifest.entries.some((entry) => entry.name === "@scientfactory/internal-fixture")).toBe(
      false,
    );
