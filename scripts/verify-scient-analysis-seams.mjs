...
const isSupportingPath = (path) =>
  ...
  path === MANIFEST_PATH ||
  path === "scripts/verify-scient-analysis-seams.mjs";
...
export function verifyScientAnalysisSeams(argv = process.argv.slice(2)) {
  ...
  const diffSignals = (manifest.analysisDiffSignals ?? []).map((signal) => new RegExp(signal, "u"));
    ...
    if (!changed.ok) {
      ...
      for (const path of changed.stdout.split("\n").filter(Boolean)) {
        ...
        if (diffSignals.some((signal) => signal.test(diff.stdout))) {
          failures.push(
            `analysis-related changed path is not classified in ${MANIFEST_PATH}: ${path}`,
          );
        }
      }
    }
  ...
  process.stdout.write(
    `Scient analysis seam check passed: ${String(ownedRoots.length)} owned roots, ${String(ownedFiles.length)} owned files, ${String(mountPaths.size)} upstream mounts${base && head ? ", current diff audited" : ""}.\n`,
  );
}
