...
export class IconExportToolResolutionError extends Schema.TaggedErrorClass<IconExportToolResolutionError>()(
  ...
  {
    reason: Schema.Literals(["configured-invalid", "not-found"]),
  ...
  override get message(): string {
    switch (this.reason) {
      ...
      case "not-found":
        return "Could not find Icon Composer's export-capable ictool. Install Icon Composer/Xcode or set ICON_COMPOSER_TOOL to Icon Composer.app/Contents/Executables/ictool.";
    }
  }
}
...
export const exportBrandIcons = Effect.fn("exportBrandIcons")(function* (checkOnly: boolean) {
  ...
  yield* Console.log(
    tool.supportsDesignGeneration
      ? `Exporting icons with Icon Composer ${tool.version}, design generation ${DESIGN_GENERATION}.`
      : `Exporting icons with Icon Composer ${tool.version}; this exporter reads the source project's embedded design generation.`,
  );
