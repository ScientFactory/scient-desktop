...
export function ClaudeInlineSetup(props: {
  ...
  const signInMethod = supportsSubscriptionSignIn
    ? "claude_subscription"
  ...
  const signIn = async () => {
    ...
    try {
      await startClaudeSignIn(props.controller, signInMethod);
    } catch (error) {
    ...
    return (
      <SetupFrame>
        ...
        <h2 className="font-semibold text-lg">
          {updating ? "Updating Claude" : "Installing Claude"}
        </h2>
        <p className="mt-1.5 text-muted-foreground text-sm">{stage.label}</p>
        <div
          aria-label={`Claude ${updating ? "update" : "installation"} progress`}
          aria-valuemax={100}
          aria-valuemin={0}
      ...
      <SetupFrame>
        ...
        {!verifying && activeConnectionOperation?.authorizationUrl ? (
          <Collapsible
            ...
            onOpenChange={(open) => {
              setShowAuthorizationCode(open);
              if (!open) setAuthorizationCode("");
            }}
            open={showAuthorizationCode}
          >
            <CollapsibleTrigger className="mx-auto flex items-center gap-1 text-muted-foreground text-xs hover:text-foreground">
              Browser showed a code?
              <ChevronDownIcon
                aria-hidden
                className="size-3 transition-transform data-[panel-open=true]:rotate-180"
              />
            </CollapsibleTrigger>
            <CollapsiblePanel className="mt-2">
              <form
                className="mx-auto flex w-fit max-w-full items-center gap-1.5"
                onSubmit={(event) => {
              ...
              >
                ...
                <Button
                  ...
                  disabled={authorizationCode.trim().length === 0}
                  size="sm"
                  type="submit"
                  variant="outline"
                >
  ...
  if (isAuthenticated) {
    if (!hasModels) {
      return (
        <StatusFrame
          body="Claude is connected but did not report an available model."
  ...
  if (props.provider.auth.required === false) {
    return (
      <StatusFrame
        title={hasModels ? "Claude is ready" : "Claude needs attention"}
        body={
          hasModels
            ? "This Claude setup does not require account sign-in."
            : "Claude did not report an available model."
        }
        warning={!hasModels}
      />
    );
