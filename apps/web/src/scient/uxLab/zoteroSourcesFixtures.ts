...
function overview(): ScientSourcesOverviewResult {
  ...
  return {
    ...
    attachmentLocations: completeRecord.attachments.map((attachment) => ({
      attachmentId: attachment.attachmentId,
      absolutePath: `/lab-fixtures/${attachment.fileName}`,
    })),
    recordDiagnostics:
      selected === "empty"
        ? []
...
export async function readZoteroLibrary(input: {
  ...
  const items = [completeRecord, metadataRecord]
    ...
    .map((item) => ({
      ...
      url: item.url,
      tags: item.tags,
      externalReference: item.externalReferences[0]!,
      fieldProvenance: item.fieldProvenance,
      pdfAvailable: item.attachments.length > 0,
      pdfFileName: item.attachments[0]?.fileName ?? null,
    }));
  return { items, start: 0, nextStart: items.length, total: items.length, hasMore: false };
}
...
export async function preflightZoteroItems(input: {
  ...
  return {
    items: library.items
      .filter((item) => input.itemKeys.includes(item.externalReference.itemKey))
      .map((candidate) => ({
        candidate,
