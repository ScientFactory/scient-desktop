...
/** Quick Chat remains visible while the ordinary sidebar is scoped to a project. */
...
/**
 * Owns the Quick Chat projection of T3's already-classified sidebar shelves.
 ...
 */
export function buildScientQuickChatSidebarModel<T extends ScientSidebarThread>(input: {
  ...
  const quickChatThreads = [
    ...pinned.general,
    ...active.general,
    ...snoozed.general,
    ...settled.general,
  ];
  const quickChatThreadKeys = new Set(quickChatThreads.map(input.getThreadKey));
  const activeQuickChatKey =
    input.activeDraft?.projectId === null
      ? `draft:${input.activeDraft.draftId}`
      : input.routeThreadKey !== null && quickChatThreadKeys.has(input.routeThreadKey)
