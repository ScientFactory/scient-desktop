...
export function ScientQuickChatIcon(props: { readonly className?: string }) {
  return (
    <MessageCircleIcon
      aria-hidden
 ...
 */
export function ScientQuickChatSection(props: {
  ...
  return (
    <>
      <section aria-label={SCIENT_QUICK_CHATS_LABEL} data-testid="scient-quick-chat-section">
        <div className="flex h-8 w-full items-center gap-1">
          <button
            type="button"
            aria-expanded={props.expanded}
            aria-controls="scient-quick-chat-list"
            onClick={props.onToggle}
            className={cn(
              "group/scient-quick-chat flex h-8 min-w-0 flex-1 cursor-pointer items-center gap-[var(--sidebar-control-gap)] rounded-[var(--control-radius)] px-[var(--sidebar-row-content-inset)] text-left text-sm font-medium text-sidebar-muted-foreground/80 transition-colors hover:bg-sidebar-row-hover hover:text-sidebar-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sidebar-ring",
            )}
          >
            <ScientQuickChatIcon className="size-4 shrink-0 group-hover/scient-quick-chat:text-sidebar-foreground" />
            <span className="min-w-0 flex-1 truncate">{SCIENT_QUICK_CHATS_LABEL}</span>
            {props.itemCount > 0 ? (
            ...
            <ChevronDownIcon
              aria-hidden
              className={cn(
                "-mr-px size-4 shrink-0 text-[var(--sidebar-icon-color)] transition-[color,transform] group-hover/scient-quick-chat:text-sidebar-foreground",
          ...
          <Tooltip>
            ...
            <TooltipPopup side="right">New quick chat</TooltipPopup>
          </Tooltip>
        </div>
        {props.expanded ? (
          <ul
            id="scient-quick-chat-list"
            aria-label={SCIENT_QUICK_CHATS_LABEL}
      ...
      <div
        aria-hidden
        data-testid="scient-quick-chat-divider"
