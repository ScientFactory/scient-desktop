...
export type ScientGeneralChatSortableBag = Pick<
...
function SortableGeneralChatRow(props: {
  readonly id: string;
  readonly children: (bag: ScientGeneralChatSortableBag) => ReactNode;
}) {
...
/**
 * Scient-owned sortable boundary for pinned General Chats. T3 continues to
 * own the actual thread row and server mutation; this component only keeps
 * the split General Chat section behaviorally equivalent to T3's pinned list.
 */
export function ScientGeneralChatPinnedList(props: {
  ...
  readonly renderRow: (
    ...
    sortable?: ScientGeneralChatSortableBag,
  ) => ReactNode;
}) {
    ...
    >
      <SortableContext items={sortableKeys} strategy={verticalListSortingStrategy}>
        {orderedThreads.map((thread) => {
          ...
          return (
            <SortableGeneralChatRow key={key} id={key}>
              {(bag) => renderRow(thread, bag)}
            </SortableGeneralChatRow>
          );
        })}
      </SortableContext>
    </DndContext>
