...
export function FirstRunGate({
  ...
  // Scient owns local assisted onboarding. Do not hydrate, probe, or hold
  // its app tree behind a second first-run decision.
  if (!enabled && !hostedStatic) return children;
  return (
    <ActiveFirstRunGate enabled={enabled} hostedStatic={hostedStatic}>
      {children}
    </ActiveFirstRunGate>
  );
}

function ActiveFirstRunGate({
  enabled,
  hostedStatic,
  children,
}: {
      ...
      <div className="flex max-w-sm flex-col items-center text-center">
        ...
        <p className="mt-2 text-sm text-muted-foreground">
          {settingsReadFailed
            ...
            : "Scient could not confirm this workspace."}
        </p>
        <Button
          className="mt-5"
          size="sm"
