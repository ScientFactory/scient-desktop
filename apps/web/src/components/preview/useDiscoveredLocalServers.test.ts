...
import { groupPreviewServers, localUrlKey, mergeServers } from "./useDiscoveredLocalServers";
...
describe("groupPreviewServers", () => {
  it("keeps configured and current-thread browser-ready servers visible", () => {
    const configured = mergeServers({
      scanner: [scannerServer({ port: 4173 })],
    ...
    expect(result.other.map((server) => server.port)).toEqual([8080]);
  });

  it("does not suggest the environment backend as a browser target", () => {
    const environmentBackend = {
      ...scannerServer({ port: 3773 }),
      source: "scanner" as const,
      terminal: { threadId: ThreadId.make("thread-current"), terminalId: "terminal-1" },
    };
    const other = { ...scannerServer({ port: 9000 }), source: "scanner" as const };

    const result = groupPreviewServers({
      servers: [environmentBackend, other],
      threadId: "thread-current",
      environmentHttpBaseUrl: "http://localhost:3773",
    });

    expect(result.relevant).toEqual([]);
    expect(result.other.map((server) => server.port)).toEqual([9000]);
  });

  it("keeps an explicitly configured environment endpoint visible", () => {
    const configuredBackend = mergeServers({
      scanner: [scannerServer({ port: 3773 })],
      configuredUrls: ["http://localhost:3773"],
    })[0]!;

    const result = groupPreviewServers({
      servers: [configuredBackend],
      threadId: "thread-current",
      environmentHttpBaseUrl: "http://localhost:3773",
    });

    expect(result.relevant.map((server) => server.port)).toEqual([3773]);
