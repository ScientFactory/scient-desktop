...
export function ThreadRouteView({ target }: { target: ThreadRouteTarget }) {
  ...
  const draftProjectUnavailable = draftProjectRef !== null && draftProject === null;
  ...
  useEffect(() => {
    if (
      target.kind !== "draft" ||
      canonicalThreadRef ||
      (draftSession && !draftProjectUnavailable) ||
      (draftProjectUnavailable && !environmentShellsBootstrapped)
    ) {
      return;
    }
  ...
  if (target.kind === "draft") {
    if (draftSession && !draftProjectUnavailable) {
      view = (
        <ChatView
          key={target.draftId}
