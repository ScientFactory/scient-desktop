...
function rememberComposerCursor(key: string, cursor: number): void {
  rememberedCursors.delete(key);
  rememberedCursors.set(key, cursor);
  if (rememberedCursors.size > 128) {
    rememberedCursors.delete(rememberedCursors.keys().next().value!);
  }
}

export function rememberedComposerCursor(key: string): number | undefined {
  return rememberedCursors.get(key);
}
...
export interface ComposerPromptEditorProps {
  /** Stable draft identity used to restore its cursor after switching queue targets. */
...
function ComposerCitationNodeView({ node, editor, getPos }: NodeViewProps) {
  ...
  const citation = node.attrs.citation as ComposerCitation;
  ...
  const onSaveComment = useCallback(
    (comment: string): boolean => {
      ...
      const currentCitation = current.attrs.citation as ComposerCitation;
      const next = withComposerCitationComment(currentCitation, comment);
      const tr = editor.state.tr.setNodeMarkup(pos, undefined, {
        ...current.attrs,
        citation: next,
        source: serializeComposerCitation(next),
      });
    ...
    >
      <AssistantCitationChip
        ...
        commentEditor={{
          ...
          ...(commentTarget?.removeOnCancel ? { mode: "create" as const } : {}),
...
function ComposerPromptEditorTiptapInner(props: ComposerPromptEditorProps) {
  ...
  useLayoutEffect(() => {
    if (!editor) return;
    const apply = () => applyComposerDirection(editor.view.dom, contentDirection);
    apply();
  ...
  useLayoutEffect(() => {
    ...
    if (draftIdentity) rememberComposerCursor(draftIdentity, normalizedCursor);
    selectionRangeRef.current = {
