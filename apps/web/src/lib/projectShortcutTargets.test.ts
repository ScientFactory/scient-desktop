...
describe("project shortcut targets", () => {
  ...
  it("falls back to the latest ordinary project from Home or an unfocused shell", () => {
    for (const focusedProjectId of [HOME_PROJECT_ID, null]) {
      expect(
        resolveNewThreadTarget({
          currentProjectId: resolveCurrentProjectTargetId(projects, focusedProjectId),
          latestUsableProjectId: resolveLatestProjectTargetId(projects, LATEST_PROJECT_ID),
        }),
  ...
  it("returns no target for stale or absent projects", () => {
    expect(
      resolveNewThreadTarget({
