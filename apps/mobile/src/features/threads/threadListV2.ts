...
export function buildThreadListV2Items(input: {
  ...
  for (const thread of input.threads) {
    // Callers pass live (unarchived) shells. The server stamps
    // settledOverride for the tail; historical projectless shells remain
    // outside Scient's current project-owned thread surface.
