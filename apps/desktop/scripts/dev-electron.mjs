...
function listStaleComputerUsePids() {
  ...
  const result = spawnSync("pgrep", ["-fal", "Scient \\(Dev\\).*(computerUseMcp\\.mjs mcp)"], {
    encoding: "utf8",
  });
...
function cleanupStaleComputerUseApps() {
  ...
  console.error(
    `[desktop-dev] Cleaning up ${stalePids.length} stale Scient (Dev) Computer Use helper process${stalePids.length === 1 ? "" : "es"} from other worktrees.`,
  );
...
function warnIfReleaseAppRunning() {
  ...
  const result = spawnSync("pgrep", ["-fal", "/Applications/Scient\\.app/Contents/MacOS/Scient"], {
    encoding: "utf8",
  });
  const output = typeof result.stdout === "string" ? result.stdout.trim() : "";
  if (!output) {
  ...
  console.error(
    "[desktop-dev] Scient is still running. Close it before testing voice in Scient (Dev), or you may be looking at the wrong app/runtime.",
  );
