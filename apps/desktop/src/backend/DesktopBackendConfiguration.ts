...
const resolvePrimaryStartConfig = Effect.fn("desktop.backendConfiguration.resolvePrimary")(
  ...
  > {
    ...
    return {
      ...
      // SCIENT-FORK: upstream gates the compile cache on `isPackaged`, but a
      // packaged app launched against a dev server is still a development
      // launch. Gating on `isDevelopment` keeps the cache off for every
      // ordinary development launch, so a dev instance never shares the
      // cache with the production app it is often run alongside. `--require`
      // rather than NODE_COMPILE_CACHE, so the setting does not leak into the
      // provider and terminal processes the backend starts.
      args: [
        ...(environment.isDevelopment ? [] : ["--require", environment.compileCachePath]),
