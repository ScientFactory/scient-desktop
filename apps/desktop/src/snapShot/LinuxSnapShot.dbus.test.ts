...
it.runIf(hasDbus)("captures through real D-Bus marshalling on a private bus", async () => {
  ...
  try {
    ...
    await server.requestName("org.gnome.Shell.Extensions.ScientSnapShot", NameFlag.DO_NOT_QUEUE);
    ...
    await snapshot.feedback!.activate("Scient");
    ...
    expect(activateTitle).toBe("Scient");
