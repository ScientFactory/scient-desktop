...
<header>↑ Drag Scient into the list above</header>
<button id="app" draggable="true" aria-label="Drag Scient to System Settings, or click to reveal in Finder"><img src="${escapeHtml(icon)}" alt="" draggable="false">Scient</button>
</main></body></html>`;
}
...
export class MacPermissionHelper {
  ...
  async show(
    ...
    if (!appIcon) throw new Error("The packaged Scient icon is missing.");
