...
it.layer(NodeServices.layer)("Antigravity installation", (it) => {
  it.effect("records a newer registry revision without downloading the same archive again", () =>
    Effect.gen(function* () {
      ...
      expect((yield* h.installation.resolve()).registryVersion).toBe("1.0.0");
      yield* h.installation.startRelease({ ...asset, registryVersion: "1.1.0" });
      expect((yield* terminalState(h.installation)).phase).toBe("succeeded");
  ...
  it.effect("does not join a different immutable artifact with the same version label", () =>
    Effect.gen(function* () {
      ...
      const started = yield* h.installation.start;
      yield* Deferred.await(entered);
      expect((yield* h.installation.startRelease(asset)).operationId).toBe(started.operationId);
      const refused = yield* h.installation
        .startRelease({ ...asset, sha256: "2".repeat(64) })
        .pipe(Effect.flip);
      expect(refused.detail).toContain("Another Antigravity release");
  ...
  it.effect("repairs only a damaged pinned generation after its replacement verifies", () =>
    Effect.gen(function* () {
      const entered = yield* Deferred.make<void>();
      const release = yield* Deferred.make<void>();
      const h = yield* makeHarness({
      ...
      yield* h.fs.writeFileString(harness, "damaged");
      yield* h.installation.start;
      yield* Deferred.await(entered);
      expect(yield* h.fs.readFileString(harness)).toBe("damaged");
      yield* Deferred.succeed(release, undefined);
      expect((yield* terminalState(h.installation)).phase).toBe("succeeded");
      yield* Deferred.await(h.stagingReleased);
      expect(yield* h.fs.readFileString(harness)).toBe(harnessContents);
      expect((yield* h.installation.resolve()).version).toBe(asset.version);
      expect(yield* h.fs.readDirectory(h.path.dirname(directory))).toEqual([asset.sha256]);
    }),
  );

  it.effect.each(["download", "verification"] as const)(
    "preserves a damaged generation when repair %s fails",
    (stage) =>
      Effect.gen(function* () {
        const h = yield* makeHarness({
          ...(stage === "download"
            ? { body: Stream.make(new Uint8Array([0])) }
            : {
                validate: () =>
                  Effect.fail(
                    new AntigravityInstallationError({
                      operation: "verify",
                      detail: "Fixture validation failed",
                    }),
                  ),
              }),
        });
        ...
        const activePath = h.path.join(h.installation.managedDirectory, "active.json");
        const active = yield* h.fs.readFileString(activePath);
        yield* h.fs.writeFileString(harness, "damaged");
        yield* h.installation.start;
        expect((yield* terminalState(h.installation)).phase).toBe("failed");
        yield* Deferred.await(h.stagingReleased);
        expect(yield* h.fs.readFileString(harness)).toBe("damaged");
        expect(yield* h.fs.readFileString(activePath)).toBe(active);
      }),
  );

  it.effect("restores the prior generation when publishing a repaired copy fails", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const h = yield* makeHarness({
        fileSystem: FileSystem.FileSystem.of({
          ...
          rename: (source, destination) =>
            source.includes(".install-") && source.endsWith("runtime")
              ? Effect.fail(
                  PlatformError.systemError({
                    _tag: "PermissionDenied",
                    module: "FileSystem",
                    method: "rename",
                    description: "fixture locked destination",
                  }),
                )
      ...
      yield* h.fs.writeFileString(harness, "damaged");
      yield* h.installation.start;
      const result = yield* terminalState(h.installation);
      expect(result.phase).toBe("failed");
      expect(result.message).toContain("previous copy was restored");
      yield* Deferred.await(h.stagingReleased);
      expect(yield* h.fs.readFileString(harness)).toBe("damaged");
      expect(yield* h.fs.readDirectory(versions)).toEqual([asset.sha256]);
    }),
  );

  it.effect("does not replace an in-use generation during repair", () =>
    Effect.gen(function* () {
      const h = yield* makeHarness({
        validate: (executable) =>
          executable.executablePath.includes(".install-")
            ? Effect.void
            : Effect.fail(
                new AntigravityInstallationError({
                  operation: "verify",
                  detail: "Fixture existing runtime failed startup",
                }),
              ),
      });
      ...
      const result = yield* terminalState(h.installation);
      expect(result.phase).toBe("failed");
      expect(result.message).toContain("Stop Antigravity sessions");
      yield* Deferred.await(h.stagingReleased);
      expect(yield* h.fs.readFileString(leased.harnessPath)).toBe(harnessContents);
