...
} from "./LegacyAntigravityDriver.ts";
  ...
  create: (createInput) =>
    Effect.gen(function* () {
      ...
      if (
        ...
        return yield* LegacyAntigravityDriver.create(createInput);
      }
      ...
      const stampBaseIdentity = withInstanceIdentity({
        instanceId,
        driverKind: DRIVER,
        displayName,
        accentColor,
      ...
      const makeRuntime = Effect.fn("AntigravityDriver.makeRuntime")(function* (
        ...
        Scope.Scope
      > {
        if (authConfigIssue !== null && !options?.initializeOnly) {
