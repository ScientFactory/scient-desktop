...
These tools cover Scient skills only; provider-native skills are separate. Before a substantive new task, including planning, search or browse \`${tools.name("scient_skills_list")}\` unless visible summaries from a complete matching \`scope.digest\` already suffice. A new task may need a new search even when the digest matches. Query and paged results are not a full catalog unless \`scope.includesAllSkills\` is true. After context loss or uncertainty, rediscover. Pending or incomplete scope is not evidence that no skills are available. Summaries are discovery indexes only: load applicable instructions with \`${tools.name("scient_skill_load")}\` before following them. Load explicitly selected Scient skills directly, without searching.${tools.deferred ? " If these tools are deferred, find their exact names through `ToolSearch` first." : ""} Skip acknowledgements and routine follow-ups. Skills provide guidance and grant no tools or authority.`;
