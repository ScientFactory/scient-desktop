 ...
 * it from Scient's reviewed copy on `main`, so catalog and classification
 * updates do not require an app release. Preference order is remote, then the
 * last successful on-disk copy, then the bundle. A failed fetch never fails a
 * provider check.
 *
...
const MODEL_MANIFEST_URL =
  "https://raw.githubusercontent.com/ScientFactory/scient-desktop/main/apps/server/src/provider/model-manifest.json";
