...
it.layer(layer)("AntigravityAdapter", (it) => {
  ...
  it.effect("serves client file reads and writes only inside the session roots", () =>
    Effect.gen(function* () {
      ...
      yield* write({
        sessionId: nativeSessionId,
        path: path.join(cwd, "..notes"),
        content: "valid",
      });
      expect(yield* fs.readFileString(path.join(cwd, "..notes"))).toBe("valid");

      if ((yield* HostProcessPlatform) !== "win32") {
        const secret = path.join(outside, "secret.txt");
        yield* fs.writeFileString(secret, "unchanged");
        yield* fs.symlink(secret, path.join(cwd, "linked.txt"));
        ...
        expect(yield* fs.readFileString(secret)).toBe("unchanged");
        yield* fs.symlink(path.join(outside, "missing.txt"), path.join(cwd, "dangling.txt"));
        const danglingWrite = yield* write({
          sessionId: nativeSessionId,
          path: path.join(cwd, "dangling.txt"),
          content: "must not create",
        }).pipe(Effect.flip);
        expect(danglingWrite._tag).toBe("AcpRequestError");
        expect(yield* fs.exists(path.join(outside, "missing.txt"))).toBe(false);
        yield* fs.symlink(outside, path.join(cwd, "linked-directory"));
        const linkedCreate = yield* write({
          sessionId: nativeSessionId,
          path: path.join(cwd, "linked-directory", "nested", "new.txt"),
          content: "must not create",
        }).pipe(Effect.flip);
        expect(linkedCreate._tag).toBe("AcpRequestError");
        expect(yield* fs.exists(path.join(outside, "nested"))).toBe(false);
      }
