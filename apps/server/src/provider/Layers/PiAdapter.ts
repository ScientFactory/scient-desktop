...
/**
 * Pi extensions own their model metadata, so normalize display-only values
 * before they cross Scient's trimmed-string RPC contract. Identity values stay
 * byte-for-byte unchanged so every returned slug still resolves in Pi's model
 * registry; malformed identities are omitted instead of breaking the catalog.
 */
export function toPiProviderModelDescriptor(
  ...
  return {
    ...
    upstreamProviderName: trimToUndefined(getProviderDisplayName(provider)) ?? provider,
    ...
    const makePiExtensionUIContext = (context: PiSessionContext): ExtensionUIContext => {
      ...
      const warnUnsupported = (method: string) => {
        ...
        offerRuntimeEvent({
          ...
          payload: {
            message: `Pi extension UI API '${method}' is not supported in Scient yet.`,
            detail: { method },
          },
          raw: {
            source: "pi.sdk.event",
      ...
      const uiContext: ExtensionUIContext = {
        ...
        setTheme() {
          return { success: false, error: "Scient does not expose Pi themes." };
        },
        getToolsExpanded() {
          return false;
        },
      ...
      Effect.gen(function* () {
        ...
        if (loadedExtensions.length > 0) {
          ...
          offerRuntimeEvent({
            ...
            payload: {
              message:
                "Pi extensions are loaded with Scient's limited UI bridge. select/confirm/input/notify/status are supported; TUI-only widgets and editor hooks are ignored.",
              detail: {
    ...
    const respondUnsupported = (threadId: ThreadId, method: string) =>
      Effect.fail(
        new ProviderAdapterRequestError({
          provider: PROVIDER,
          method,
          detail: `Pi does not expose Scient approval/user-input requests for thread ${threadId}.`,
        }),
      );
    ...
    const listModels: NonNullable<PiAdapterShape["listModels"]> = (input) =>
      Effect.tryPromise({
        try: async () => {
          ...
          const models = getPiDiscoverableModels(services.modelRegistry).flatMap((model) => {
            const descriptor = toPiProviderModelDescriptor(
              model,
              services.modelRegistry.getProviderDisplayName.bind(services.modelRegistry),
            );
