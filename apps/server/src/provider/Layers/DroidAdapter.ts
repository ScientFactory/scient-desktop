...
const DROID_PLAN_MODE_PROMPT_PREFIX = [
  "Scient Droid plan mode is active.",
        ...
        Effect.gen(function* () {
          ...
          if (resumeSessionId !== undefined && started.sessionSetupMethod === "new") {
            return yield* new ProviderAdapterRequestError({
              ...
              detail:
                "Droid could not resume the requested native session. Scient refused the fresh fallback to avoid silently losing conversation context.",
            });
          }
          ...
          Effect.gen(function* () {
            if (!(yield* runtime.supportsSessionFork)) {
              return yield* new ProviderAdapterValidationError({
                ...
                operation: "forkThread",
                issue:
                  "This Droid ACP version does not advertise session/fork; Scient will rebuild the fork from its retained transcript.",
              });
            }
        ...
        const forked = activeSource
          ...
          : yield* Effect.gen(function* () {
              ...
              const runtime = yield* makeDroidAcpRuntime({
                ...
                clientInfo: { name: "Scient Fork", version: "0.0.0" },
              });
      ...
      discoveryLock.withPermits(1)(
        Effect.gen(function* () {
          ...
          const runtime = yield* makeDroidDiscoveryRuntime({
            ...(input.binaryPath ? { binaryPath: input.binaryPath } : {}),
            cwd,
            clientName: "Scient Command Discovery",
          });
          yield* runtime.start();
