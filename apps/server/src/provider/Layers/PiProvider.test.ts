...
const settings = Schema.decodeSync(PiSettings)({ binaryPath: "fake-pi", enabled: true });
...
const unusedClientMethods = {
  ...
  close: () => Effect.void,
  getThinkingLevels: () => Effect.die("unused"),
  getSessionStats: () => Effect.die("unused"),
  clearQueue: () => Effect.die("unused"),
      ...
      Effect.succeed({
        ...
        Effect.tap(() =>
          Effect.sync(() => {
            ...
            for (const arg of ["--no-context-files", "--no-extensions", "--no-tools"])
