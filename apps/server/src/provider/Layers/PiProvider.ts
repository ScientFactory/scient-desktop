...
const models = (settings: PiSettings, discovered = mapPiDiscoveredModels([])) =>
  providerModelsFromSettings(discovered, [], {
    optionDescriptors: [],
  });
      ...
      buildServerProvider({
        ...
        probe: {
          ...
          message: settings.enabled
            ...
            : "Pi is disabled in Scient settings.",
        },
      }),
    ),
  );
...
export const checkPiProviderStatus = Effect.fn("checkPiProviderStatus")(function* (
  ...
  return buildServerProvider({
    ...
    probe: {
      ...
      message:
        discovered.length > 0
          ? `Pi reported ${discovered.length} available model${discovered.length === 1 ? "" : "s"}. Authentication is model-specific and verified when used. Full access only; no native sandbox.`
          : "Pi is available, but it did not report any models. Configure a model provider with Pi, then refresh. Full access only; no native sandbox.",
    },
  });
});
