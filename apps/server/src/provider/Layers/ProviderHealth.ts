...
function parseCursorAuthStatusFromOutput(result: CommandResult): {
  ...
  if (result.code === 0) {
    return {
      status: "warning",
      authStatus: "unknown",
      message: "Cursor Agent is installed, but Scient could not verify authentication status.",
    };
  }
  ...
  Effect.gen(function* () {
    ...
    if (Option.isNone(modelsProbe.success)) {
      return {
        ...
        checkedAt,
        message:
          "Cursor Agent is authenticated, but model discovery timed out before Scient could verify available models.",
      } satisfies ServerProviderStatus;
    }
