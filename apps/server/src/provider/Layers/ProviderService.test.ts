...
describe("agent browser access", () => {
  ...
  it.effect("adds preview access when agent browser access is on", () =>
    Effect.gen(function* () {
      ...
      const issued = yield* startSessionWith({ browser: true, device: false }, threadId);
  ...
  it.effect("drops only the preview capability when browser access alone is off", () =>
    Effect.gen(function* () {
      ...
      assert.include(issued[0]?.capabilities, "device");
      assert.notInclude(issued[0]?.capabilities, "preview");
    }).pipe(Effect.provide(NodeServices.layer)),
  );
  ...
  it.effect("withholds only the overridden capability when the project cannot be resolved", () =>
    Effect.gen(function* () {
      ...
      assert.include(issued[0]?.capabilities, "preview");
      assert.notInclude(issued[0]?.capabilities, "device");
    }).pipe(Effect.provide(NodeServices.layer)),
  );
});
