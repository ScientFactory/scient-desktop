...
export const TRIAGE_PLAYBOOK = `# Scient triage playbook

You are a support engineer for Scient (https://github.com/ScientFactory/scient-desktop-next), working
...
Fetch https://raw.githubusercontent.com/ScientFactory/scient-desktop-next/main/.github/triage/PLAYBOOK.md.
...
cache directory named in the context file, one subdirectory per commit hash:
    ...
    git clone --depth 1 --filter=blob:none --branch <release-tag> \\
      https://github.com/ScientFactory/scient-desktop-next <source-cache-dir>/<hash>
...
Search existing issues in ScientFactory/scient-desktop-next (use \`gh\`, or the public GitHub search
...
Do not patch the Scient source as a fix. A good issue with strong repro steps
...
- If \`gh\` is not authenticated, offer \`gh auth login\`, or build a prefilled
  https://github.com/ScientFactory/scient-desktop-next/issues/new URL with title and body query
...
export const buildTriageLaunchPrompt = (promptFilePath: string) =>
  `Read the file "${promptFilePath}" and follow its instructions exactly: it is your Scient triage playbook, and it starts with asking the user what went wrong.`;

/** The full seed prompt, written to `prompt.md` in the triage scratch dir. */
export const buildTriageSeedPrompt = (contextFilePath: string) => `A Scient user is \
...
export const buildTriageContext = (input: TriageContextInput) => `# Scient triage context
...
- Repo: https://github.com/ScientFactory/scient-desktop-next
