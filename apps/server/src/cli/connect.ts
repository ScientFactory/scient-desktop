    ...
    runCloudCommand(
      ...
      Effect.gen(function* () {
        ...
        if (background) {
          ...
          yield* Console.log(
            platform === "darwin"
              ? "\n✓ Background service ready\n\nScient is set to run while you are logged in to this Mac. The server establishes the remote connection on startup."
              : "\n✓ Background service ready\n\nScient is set to keep running after you log out. The server establishes the remote connection on startup.",
          );
          return;
        }
        const serveCommand = yield* resolveCliCommand("serve");
