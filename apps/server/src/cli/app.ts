...
export class DesktopAppSshUnsupportedError extends Schema.TaggedErrorClass<DesktopAppSshUnsupportedError>()(
  ...
  override get message(): string {
    return "`t3 app` only controls the Scient desktop app on the same machine. It cannot run over SSH.";
  }
}
...
export class DesktopAppUnreachableError extends Schema.TaggedErrorClass<DesktopAppUnreachableError>()(
  ...
  override get message(): string {
    return "Could not reach the Scient desktop app. Start or update Scient on this machine, then run `t3 app` again. A running Scient server is not enough.";
  }
}
...
export class DesktopAppRequestFailedError extends Schema.TaggedErrorClass<DesktopAppRequestFailedError>()(
  ...
  override get message(): string {
    return `Scient could not open ${this.workspaceRoot} (${this.code}).`;
  }
}
...
const runAppCommand = Effect.fn("cli.app")(function* (flags: {
  ...
  const configuredEnvironmentBaseDir =
    environment.scientNextHome?.trim() ||
    (!SCIENT_DESKTOP_IDENTITY.safetyEnvelopeEnabled ? environment.t3Home?.trim() : undefined);
  const configuredBaseDir = Option.getOrUndefined(flags.baseDir) ?? configuredEnvironmentBaseDir;
  ...
  const fallbackAddress = allowDevFallback
    ? resolveAddress(SCIENT_DESKTOP_IDENTITY.developmentUserDataDirName)
    : undefined;
  ...
  yield* Console.log(`Opened ${workspaceRoot} in Scient.`);
});
...
export const appCommand = Command.make("app", {
  ...
  Command.withDescription("Open a project in the running Scient desktop app."),
