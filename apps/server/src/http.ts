...
export const assetFileResponse = Effect.fn("assetFileResponse")(function* (
  ...
  const headers: Record<string, string> = {
    ...
    "Content-Type":
      asset.mimeType ??
      Option.getOrElse(Mime.getType(asset.path), () => "application/octet-stream"),
...
// Registered for every method so the handler can answer HEAD and refuse the
// rest with a real 405 plus an Allow header, rather than letting the router
// 404 them. Upstream registers GET only and inlines the body.
