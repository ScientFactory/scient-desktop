...
describe("Grok connection actions", () => {
  ...
  it.effect("does not sign out when active Grok sessions cannot be stopped", () =>
    Effect.gen(function* () {
      ...
      const actions = withGrokSessionShutdown(
        {
          methods: ["grok_account"],
          start: () => Effect.die("must not start"),
          disconnect: Ref.update(logoutCalls, (count) => count + 1),
        },
      ...
      expect(failure.message).toBe(
        "Scient could not stop active Grok sessions before signing out.",
      );
