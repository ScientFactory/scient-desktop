import {
  QUICK_CHAT_MOVE_SESSION_STOP_PENDING,
...
export type ScientQuickChatMoveRejection =
...
export function validateScientQuickChatMove(input: {
...
}): ScientQuickChatMoveRejection | null {
  if (input.target.deletedAt !== null) {
    return {
      code: "destination-deleted",
      detail: `Project '${input.target.id}' is deleted and cannot receive a Quick Chat.`,
    };
  }
  ...
  if (input.thread.session !== null && input.thread.session.status !== "stopped") {
    return {
      ...
      detail: `${QUICK_CHAT_MOVE_SESSION_STOP_PENDING}: Thread '${input.thread.id}' provider session must be stopped before relocation.`,
    };
  }
