...
export const make = (options: GeneratedDocumentStoreOptions = {}) =>
  Effect.gen(function* () {
    ...
    const validator = yield* Effect.acquireRelease(
      Effect.sync(() =>
        createPdfValidationRuntime({ workerUrl: packagedPdfValidationWorkerUrl() }),
      ),
      ...
      yield* writeFileStringAtomically({ filePath, contents: `${encodeBinding(binding)}\n` }).pipe(
        ...
        Effect.mapError((cause) =>
          makeStoreError(
            "write-binding",
            "filesystem",
            "Unable to persist document binding.",
            cause,
          ),
        ),
      );
    ...
    const readRetentionIndex = optionalOnNotFound(
      fileSystem.readFileString(retentionIndexPath),
    ).pipe(
      Effect.map(
        Option.match({
