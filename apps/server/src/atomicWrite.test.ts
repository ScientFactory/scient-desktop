...
describe.skipIf(process.platform === "win32")("private atomic writes", () => {
  it("preserves binary contents without string coercion", async () => {
    const directory = fs.mkdtempSync(path.join(os.tmpdir(), "synara-atomic-binary-"));
    const filePath = path.join(directory, "secret.bin");
    const contents = new Uint8Array([0, 255, 1, 128, 10]);
