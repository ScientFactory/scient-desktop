...
export const reconcileProviderSessions = Effect.gen(function* () {
  ...
  const continueAfterRestartFor = (projectId: ProjectId | null) =>
    Option.isSome(restartSettings)
      ? projectId === null
        ? restartSettings.value.continueThreadsAfterServerUpdate
    ...
    const startup = Effect.gen(function* () {
      ...
      yield* runStartupPhase(
        ...
        Effect.gen(function* () {
          ...
          yield* launchAnalyticsEventObservers.pipe(Scope.provide(reactorScope));
        }),
      );
          ...
          if (serverConfig.startupPresentation === "headless") {
            ...
            if (serverConfig.mode !== "desktop") {
              yield* Effect.logInfo(
                "Authentication required. Open Scient using the pairing URL.",
