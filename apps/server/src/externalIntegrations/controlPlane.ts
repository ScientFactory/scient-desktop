...
export const makeExternalIntegrationControlPlane = Effect.gen(function* () {
  ...
  const createPending: ExternalIntegrationControlPlane["createPending"] = (input) =>
    Effect.try({
      try: () => validateCreate(input),
        ...
        cause instanceof ExternalIntegrationControlError
          ? cause
          : controlError("invalid_configuration", "External integration configuration is invalid."),
    }).pipe(
            ...
            Effect.gen(function* () {
              yield* sql`
                INSERT INTO scient_external_integrations (
                  ...
                  ${input.rateLimit.maxRequests}, ${input.rateLimit.windowMs}, ${now},
                  0, ${now}, ${now}
                )
              `;
              ...
              yield* sql`
                ...
                ) VALUES (${randomUUID()}, ${integrationHash}, 'created', 'recorded', 'pending_pairing', ${now})
              `;
        ...
        .withTransaction(
          Effect.gen(function* () {
            ...
            if (
              ...
              return yield* controlError("pairing_denied", "Pairing proof was rejected.");
            }
            yield* sql`
          ...
          `;
            ...
            INSERT INTO scient_external_integration_security_events (
              event_id, integration_hash, event_type, outcome, reason_code, occurred_at
            ) VALUES (${randomUUID()}, ${integrationHash}, 'paired', 'recorded', 'owner_pairing_completed', ${now})
          `;
          }),
        )
        .pipe(
      ...
      return sql
        .withTransaction(
          Effect.gen(function* () {
            ...
            if (row === undefined) {
              denial = controlError("integration_not_found", "External integration was not found.");
            ...
            if (denial === null && !projects.some((entry) => entry.projectHash === projectHash)) {
              denial = controlError("project_scope_denied", "Project is outside the exact grant.");
            }
            if (
              denial === null &&
              threadHash !== null &&
            ...
            if (denial === null && !capabilities.some((entry) => entry.capability === capability)) {
              denial = controlError("capability_denied", "Read capability is not granted.");
            }

            let nextWindowStart = row?.rateWindowStartedAt ?? now;
            let nextCount = row?.rateWindowCount ?? 0;
            if (row !== undefined && now - row.rateWindowStartedAt >= row.rateLimitWindowMs) {
              nextWindowStart = now;
              nextCount = 0;
            }
  ...
  const releaseRead: ExternalIntegrationControlPlane["releaseRead"] = (
    admission,
    releasedAt = Date.now(),
  ) =>
    sql
  ...
  const revoke: ExternalIntegrationControlPlane["revoke"] = (
    externalIdentity,
    revokedAt = Date.now(),
  ) =>
    validatedEffect(() => {
