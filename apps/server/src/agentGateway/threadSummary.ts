 ...
 * `scient_list_threads`, and truncated/paginated message views for
 * `scient_read_thread`. Kept pure so the shaping rules are unit-testable.
 *
...
function toAgentThreadMessageSummary(
...
export function paginateThreadMessages(input: {
  ...
  const messages = input.messages
    ...
    .map((message, offset) =>
      toAgentThreadMessageSummary(message, startInclusive + offset, maxChars),
    );
  return {
    messages,
    totalMessages: total,
