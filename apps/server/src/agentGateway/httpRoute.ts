/**
 * HTTP route for the Scient agent gateway MCP endpoint.
 *
...
const postRouteLayer = HttpRouter.add(
  ...
  Effect.gen(function* () {
    ...
    // The route layer is always registered so the layer graph stays static, but
    // the endpoint is absent unless the operator enables the gateway. 404 (not
    // 401) so a disabled instance is indistinguishable from one that never had
    // the route.
...
// server is stateless, so both are explicit non-endpoints. Both mirror POST's
// disabled-state behavior: they 404 when the gateway is off so a disabled
// instance is indistinguishable from one that never registered the route, and
// only surface the 405 once the operator has enabled the feature.
