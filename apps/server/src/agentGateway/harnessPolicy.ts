 ...
 * thread-scoped Scient MCP connection.
 *
...
export const SYNARA_HARNESS_POLICY_MARKER = `[Scient harness policy ${SYNARA_HARNESS_POLICY_VERSION}]`;
...
export function renderSynaraHarnessPolicy(capabilities: SynaraHarnessCapabilities): string {
  const controlPolicy = capabilities.gatewayControlAvailable
    ? [
        "Observe sibling Scient threads in your project with scient_context (your identity and capabilities), scient_list_projects, scient_list_threads, scient_read_thread, and scient_wait_for_threads.",
        "Drive sibling threads with scient_send_message (queue a message, or steer a running turn) and scient_interrupt_thread (stop a running turn). Drive tools work only while your own turn is active.",
        ...
        "When you need another thread's outcome, call scient_wait_for_threads with its thread ids and pinned run ids, wait for every requested result, then synthesize the outcomes.",
        "scient_wait_for_threads timeouts only report progress; they never retry, replace, cancel, or create work.",
    ...
    : [
        "Scient MCP control is unavailable in this provider session. Do not claim that you can observe, create, or change Scient threads, projects, or automations.",
        "Provider-native subagent or Task tools do not create or observe Scient threads. If the user explicitly requests Scient resource management, explain that this session cannot perform it.",
      ];

  return [
    SYNARA_HARNESS_POLICY_MARKER,
    "You are running inside Scient. Scient is the host and harness for this session.",
    ...controlPolicy,
  ].join("\n");
}
...
// Providers with a thread-scoped Scient MCP connection actually wired and
