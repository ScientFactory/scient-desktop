...
describe("PullRequestSyncReactor", () => {
  it.effect("does not refresh links for historical null-project threads", () =>
    Effect.scoped(
      Effect.gen(function* () {
        const fixture = yield* makeHarness({
          snapshot: makeSnapshot([
            makeThread("retired", { projectId: null, pullRequests: [makeLink(42)] }),
          ]),
        });
        yield* startAndSweep(fixture).pipe(Effect.provide(fixture.layer));
