...
it.layer(NodeServices.layer)("pull request link decider", (it) => {
  it.effect("does not attach new links to a historical null-project thread", () =>
    Effect.gen(function* () {
      const model = makeReadModel([]);
      const error = yield* decideOrchestrationCommand({
        command: {
          type: "thread.pull-request.link",
          commandId: CommandId.make("cmd-link-retired"),
          threadId: THREAD_ID,
          host: "github.com",
          repository: "t3tools/t3code",
          number: 42,
        ...
        readModel: {
          ...model,
          threads: model.threads.map((thread) => ({ ...thread, projectId: null })),
        },
      }).pipe(Effect.flip);
      expect(error).toMatchObject({
        _tag: "OrchestrationCommandInvariantError",
        detail: "Pull request links require a thread with an owning project.",
      });
    }),
  );
