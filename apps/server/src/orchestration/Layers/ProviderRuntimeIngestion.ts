...
const make = Effect.gen(function* () {
  ...
  const appendGeneratedImagesToMessage = Effect.fn("appendGeneratedImagesToMessage")(
    function* (input: {
      ...
      const current = yield* getThreadMessageById(input.threadId, input.messageId);
  ...
  const flushGeneratedImagesForTurn = Effect.fn("flushGeneratedImagesForTurn")(function* (input: {
    ...
    const messages = yield* projectionThreadMessages.listByThreadId({ threadId: input.threadId });
    const existingTarget = messages
      .toReversed()
    ...
    Effect.gen(function* () {
      ...
      if (generatedImageArtifact) {
        ...
        if (turnId) {
          ...
          const latestTurn = Option.getOrUndefined(
            yield* projectionSnapshotQuery.getThreadShellById(thread.id),
          )?.latestTurn;
          const turnAlreadyTerminal =
            latestTurn?.turnId === turnId && latestTurn.state !== "running";
