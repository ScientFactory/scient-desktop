...
it.layer(
  ...
  it.effect("projects additive agent provenance onto the triggering user message", () =>
    Effect.gen(function* () {
      const eventStore = yield* OrchestrationEventStore;
      const projectionPipeline = yield* OrchestrationProjectionPipeline;
      const sql = yield* SqlClient.SqlClient;
      ...
      const createdAt = "2026-07-28T05:05:00.000Z";
      ...
      const rows = yield* sql<{
        ...
        readonly dispatchSource: string | null;
      }>`
        SELECT
      ...
      assert.deepEqual(rows, [{ dispatchOrigin: null, dispatchSource: "agent" }]);
    }),
  );
});
