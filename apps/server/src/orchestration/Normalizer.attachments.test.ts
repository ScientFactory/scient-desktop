...
describe("question attachments", () => {
  it.effect("rejects a historical thread file as a question upload without removing it", () =>
    Effect.gen(function* () {
      const config = yield* ServerConfig.ServerConfig;
      const id = `other-thread-${attachmentUuid}-txt`;
      const ownedPath = NodePath.join(config.attachmentsDir, `${id}.txt`);
      NodeFS.writeFileSync(ownedPath, "report");
      const failure = yield* normalizeDispatchCommand({
        type: "thread.user-input.respond",
        commandId: CommandId.make("answer-owned-file"),
        threadId: ThreadId.make("thread-1"),
        requestId: ApprovalRequestId.make("request"),
        answers: { q: "" },
        createdAt: "2026-08-01T00:00:00.000Z",
        attachmentsByQuestionId: {
          q: [{ type: "file", id, name: "report.txt", mimeType: "text/plain", sizeBytes: 6 }],
        },
      }).pipe(Effect.flip);
      expect(failure.message).toContain("attachment must be a pending upload");
      expect(NodeFS.readFileSync(ownedPath, "utf8")).toBe("report");
    }).pipe(Effect.provide(testLayer)),
  );
