...
Status: inherited upstream implementation and deployment reference. Scient's cloud/relay and
mobile publication holds remain in force. This guide does not authorize provisioning Firebase
credentials, applying a live relay migration, or publishing an Android build. See
[Connect setup](./connect-setup.md) for the existing Scient activation boundary.
