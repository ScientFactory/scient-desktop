...
| `ChatMarkdown.tsx` and `FileMarkdownPreview.tsx`                   | The editor's code header and figure captions must match the established preview exactly.    | The code-block title and copy/wrap actions live once in `scient/presentation`; when T3 changes that header, port the change there instead of re-inlining it. `imageCaptions` is an opt-in prop; chat rendering keeps its defaults.                  |
| `index.css`                                                        | Chat, file preview, and editor share one quiet code-card surface and table separators.      | Additive only: the two `--markdown-code-block-*` tokens sit beside `--code-background`, and every override lives in the trailing Scient block after the inherited rule it refines. Never edit inherited rule bodies; the style test pins the order. |
| `ui/menu.tsx` and `ui/popover.tsx`                                 | Editor popups anchor inside a scrolling document and must stay within its bounds.           | Pass optional Base UI positioner props (`collisionAvoidance`, `collisionBoundary`, `side`) through unchanged; keep upstream defaults when the prop is absent.                                                                                       |
| `media/MediaActions.tsx`                                           | Editor figures reuse the host's authenticated asset URL resolution for copy and save.       | Keep `useMediaActionUrl` as a thin extraction over the existing `useMediaActions`; URL policy stays in the owned `mediaActionUrl.ts`.                                                                                                               |
