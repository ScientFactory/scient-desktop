...
Quick Chat is the home for conversations that have not been placed in a
project. It is an ordinary orchestration thread with `projectId: null`, not a
second chat engine or a hidden pseudo-project.

Scient supports multiple Quick Chats per environment. They appear in a
...
Quick Chat reveals it once without preventing the user from collapsing it
again. Its plain `+` action creates a Quick Chat directly in the active
capable environment, with the primary capable environment as fallback.

Quick Chat can open the right panel (or its responsive sheet) for browser
...
Quick Chat and preserves the same environment root.

## Ownership boundary

ScientFactory owns Quick Chat end to end: product policy, capability
...
- `packages/client-runtime/src/scient/quickChat.ts` for cross-client product
  identity and capability interpretation;
- `apps/server/src/scient/quickChat/` for server capability authority;
- `apps/web/src/scient/quickChat/` for workspace, terminal, and sidebar
  projection policy;
- `apps/web/src/components/scient-quick-chat/` for navigation, disclosure,
  relocation, presentation, and compatibility controllers; and
- `apps/mobile/src/features/scient-quick-chat/` for mobile-specific policy
  and a discriminated Quick Chat group that never fabricates a project.
...
[`scient-quick-chat-seams.json`](../../scient-quick-chat-seams.json) is the
...
projectless decision. `scripts/verify-scient-quick-chat-seams.mjs` rejects a
...
The authoritative Quick Chat root is the durable thread `workspaceRoot`,
...
Quick Chat workspace must never be used as evidence that Git, checkpoint,
...
a merge automatic. Every upstream pass must rerun Quick Chat creation,
...
Quick Chat candidate produces the same five conflict files and adds none.
