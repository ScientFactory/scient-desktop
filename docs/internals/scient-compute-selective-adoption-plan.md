...
| File-first setup and connection       | Adopt                 | Keep setup in the file toolbar, but derive Install/Connect/Repair/Use helper from typed status and ownership. Never infer it from visible copy.                        |
...
| Runtime picker labels                 | Adopt with correction | Hide paths by default; append a short path suffix only when version and provenance labels collide. Full paths stay available as titles/details.                        |
...
| Test passed state                     | Adopt with correction | Bind the result to the exact runtime fingerprint and clear it automatically when executable, version, or problem state changes.                                        |
