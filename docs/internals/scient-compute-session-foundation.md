...
The candidate was subsequently realigned onto `origin/main` snapshot
`2bb6d79b0c` after the Phase 5 plan and its durable architectural corrections
were complete. Exact implementation-and-architecture checkpoint `b26d93f139`
passes 100 compute, 317 contract, 641 client-runtime, 83 Python-bridge, 3,734
web, and 3,745 server tests; the server suite skips 35 tests behind explicit
gates. All affected typecheck boundaries, targeted formatting and lint, seam
and brand checks, the production web/server/desktop build, byte-identical
bridge staging, release smoke, and Electron smoke also pass. Hosted checks for
the newly published rebased head remain required; the earlier hosted evidence
is historical evidence for `fcf7c22fab`, not a qualification shortcut.
...
| Stable rebased candidate                       | Implementation-and-architecture checkpoint `b26d93f139` contains `origin/main` snapshot `2bb6d79b0c` in its ancestry and passes the affected local qualification gates: 100 compute, 317 contract, 641 client-runtime, 83 bridge, 3,734 web, and 3,745 server tests; affected typechecks; targeted format/lint; seam/brand; production build with exact bridge staging; release smoke; and Electron smoke. The last complete hosted qualification remains `fcf7c22fab` on older `main` snapshot `015bfd8cda`; hosted checks must pass again on the newly published head.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | Stable and locally requalified; hosted pending |
...
| Packaged cross-platform and release acceptance | Checkpoint `b26d93f139` passes local production web, desktop, and server builds, exact bridge staging, release smoke, and Electron smoke. All configured required hosted checks passed older exact head `fcf7c22fab`; the rebased published head must pass them again. The release-qualification track's installed-app tests on supported platforms and explicit release approval remain.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | Local assembly passed; hosted/release pending  |
