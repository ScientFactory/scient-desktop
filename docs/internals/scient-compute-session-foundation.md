...
The final planning-time replay advances to `origin/main` snapshot `fdd4d66966`.
That mainline delta changes only CI workflows and their internal documentation;
rebased implementation-and-architecture checkpoint `19d811f934` is otherwise
byte-identical to `b26d93f139`. The newly published head still requires its own
hosted checks. Evidence is always exact-snapshot evidence: neither result
qualifies Windows or a later rebased head.
...
| Stable rebased candidate                       | Implementation-and-architecture checkpoint `19d811f934` contains `origin/main` snapshot `fdd4d66966`. Its product source is byte-identical to locally qualified checkpoint `b26d93f139`; the only mainline delta is CI workflow/documentation cleanup. Older documentation closeout `9f5abf3fad` passed every configured hosted check, including all three server shards and the macOS/Linux real-kernel matrix on Python 3.10 and 3.12. The final replay still requires exact-head hosted checks.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | Stable and locally requalified; hosted pending |
...
| Packaged cross-platform and release acceptance | Product-source checkpoint `b26d93f139` passes local production web, desktop, and server builds, exact bridge staging, release smoke, and Electron smoke; rebased checkpoint `19d811f934` changes only CI workflows/documentation relative to that tree. All configured required hosted checks pass older closeout `9f5abf3fad`; the final replay must pass them again. The release-qualification track's installed-app tests on supported platforms and explicit release approval remain.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | Local assembly passed; hosted/release pending  |
