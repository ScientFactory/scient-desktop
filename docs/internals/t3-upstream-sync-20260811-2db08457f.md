...
| ----------- | ------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------ |
| `b30a9bc41` | Theme-aware environment stage artwork                   | Adapt: keep Scient's minimal stage treatment and brand watermark; adopt upstream focus-ring helper API and theme-owned stage CSS token defaults. |
...
1. `SidebarStageBackdrop.tsx` / test: retained Scient stage art (`data-scient-stage`,
   Scient watermark). Added upstream `resolveSidebarStageFocusRingOffsetClass`
   for API compatibility without adopting T3 SVG night/blueprint scenes.
...
4. `index.css`: kept Scient typography import and Scient stage brand CSS; also
   adopted upstream per-theme-id OKLCH stage token overrides so future theme
   refreshes remain upstream-shaped.
