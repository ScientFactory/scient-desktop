...
Shift keys together. Scient attaches the image to your draft and brings itself forward. If no thread
is open it starts a draft in the current project.

Pressing the shortcut while Scient is in front captures Scient itself.
...
a modifier pair such as Command+Command or Ctrl+Ctrl, or a key chord. Scient refuses shortcuts
...
terminal contents. If an app is slow to answer, Scient attaches the screenshot without the data
rather than waiting.
...
**GNOME.** Install the bundled **Scient SnapShots** extension during setup. It is installed
...
shortcut does not fire, check Scient under **System Settings** > **Keyboard** > **Shortcuts**. If a
...
**Review changes**. Scient shows the exact change it will make to your Hyprland config. **Save
...
**Niri.** Choose a shortcut and select **Review changes**. Scient shows the exact binding it will
...
**Other Wayland desktops.** Scient uses the desktop's screenshot portal when it can capture the
