...
Oh My Pi uses the `omp` executable you install. Scient does not download it or sign in to it.
Full access is the only runtime mode, and Oh My Pi's own approval mode is explicit `yolo`.
